def make_smiley_face():
    ...
    mouth_block = max(blocks, key=lambda block: block.size[0])  # assuming the longest dimension is along the x-axis
    blocks.remove(mouth_block)  # Remove the mouth block from blocks list
    # Define the center position for the smiley face
    workspace = Workspace()
    center_of_workspace = Point3D((workspace.bounds[0, 0] + workspace.bounds[0, 1]) / 2,
